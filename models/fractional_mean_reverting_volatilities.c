...
void fractional_ou(command_line_options* clo) {
    int sim,step;
    ...
    for(sim = 0; sim < clo->number_of_simulations; sim++) {
        ...
        for(step = 0; step < clo->number_of_steps; step++) {
            new_volatilities = old_volatilities + 
                           ...
                           clo->variance_variance * fractional_brownian_samples[step];
...
void fractional_cir(command_line_options* clo) {
    int sim,step;
    double old_volatilities, new_volatilities, old_price, new_price, dt;
    double h = 0.5; 
    int method = 0;
    ...
    for(sim = 0; sim < clo->number_of_simulations; sim++) {
        ...
        for(step = 0; step < clo->number_of_steps; step++) {
            new_volatilities = old_volatilities + 
                           clo->mean_reversion_rate * (clo->mean_reversion_level - old_volatilities) * dt +
                           clo->variance_variance * sqrt(old_volatilities) * fractional_brownian_samples[step];
